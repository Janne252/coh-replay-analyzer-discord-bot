...
{
    public static class GenerateScenarioPreviewImages
    {
        ...
        /// <summary>
        /// Specifically separated output of warnings. Used to display all relevant info outside of less relevant log output.
        /// </summary>
        static List<string> Output = new List<string>();

        public static void Run(string CoH2ArchivesSourceRootPath, string CachedCustomScenariosSourceRootPath, string ScenarioPreviewImageDestinationRootPath, string ScenarioSourceIconsRootPath)
        {
            Output.Clear();
            var archives = new List<string>();

            archives.AddRange(
                Directory.GetFiles(CoH2ArchivesSourceRootPath, "*.sga", SearchOption.AllDirectories)
            );
            archives.AddRange(
                Directory.GetFiles(CachedCustomScenariosSourceRootPath, "*.sga", SearchOption.AllDirectories)
            );
            ...
            if (Directory.Exists(ScenarioPreviewImageDestinationRootPath))
                Directory.Delete(ScenarioPreviewImageDestinationRootPath, true);

            Directory.CreateDirectory(ScenarioPreviewImageDestinationRootPath);
            ...
            foreach (var filename in archives)
            {
                ...
                {
                    Console.WriteLine($"\t{archive.Name} excluded.");
                    continue;
                }
                ...
                foreach (var scenario in scenarioFolders)
                {
                    ...
                    foreach (var icon in icons)
                    {
                        ...
                        {
                            Output.Add($"Ignoring icon {icon.EbpName} for {scenario.ScenarioName} in {scenario.Folder.Archive}");
                            continue;
                        }

                        var iconImage = GetScenarioIconImage(icon, ScenarioSourceIconsRootPath);
        ...
        /// <returns></returns>
        private static Image GetScenarioIconImage(ScenarioFolder.ScenarioIcon icon, string ScenarioIconsSourceRootPath)
        {
            ...
            iconKey = Path.Join(ScenarioIconsSourceRootPath, $"{ScenarioIconsFilenameMap[iconKey]}.png");
